...
namespace UtilORama4
{
	public partial class frmVamp //: Form
	{
		...
		public int songMilliseconds = 0;
		public int songCentiseconds = 0;
		...
		private void ExportSelectedVampsToxLights(string fileName)
		{
			...
			if (allInOne)
			{
				writer = Begin_xTimingsFile(fileName);
			...
			if (chkBarsBeats.Checked)
			{
				...
				if (VampBarBeats.xBars != null)
				{
					if (VampBarBeats.xBars.Markers.Count > 0)
					{
						...
						if (!allInOne)
						{
							writer = Begin_xTimingsFile(fileBaseName + " - " + VampBarBeats.xBars.Name + exten);
							writeCount++;
						}
				...
				if (chkBeatsFull.Checked)
				{
					if (VampBarBeats.xBeatsFull != null)
					{
						if (VampBarBeats.xBeatsFull.Markers.Count > 0)
						{
							...
							if (!allInOne)
							{
								writer = Begin_xTimingsFile(fileBaseName + " - " + VampBarBeats.xBeatsFull.Name + exten);
								writeCount++;
							}
				...
				if (chkBeatsHalf.Checked)
				{
					if (VampBarBeats.xBeatsHalf != null)
					{
						if (VampBarBeats.xBeatsHalf.Markers.Count > 0)
						{
							...
							if (!allInOne)
							{
								writer = Begin_xTimingsFile(fileBaseName + " - " + VampBarBeats.xBeatsHalf.Name + exten);
								writeCount++;
							}
				...
				if (chkBeatsThird.Checked)
				{
					if (VampBarBeats.xBeatsThird != null)
					{
						if (VampBarBeats.xBeatsThird.Markers.Count > 0)
						{
							...
							if (!allInOne)
							{
								writer = Begin_xTimingsFile(fileBaseName + " - " + VampBarBeats.xBeatsThird.Name + exten);
								writeCount++;
							}
				...
				if (chkBeatsQuarter.Checked)
				{
					if (VampBarBeats.xBeatsQuarter != null)
					{
						if (VampBarBeats.xBeatsQuarter.Markers.Count > 0)
						{
							...
							if (!allInOne)
							{
								writer = Begin_xTimingsFile(fileBaseName + " - " + VampBarBeats.xBeatsQuarter.Name + exten);
								writeCount++;
							}
			...
			if (chkNoteOnsets.Checked)
			{
				if (VampNoteOnsets.Timings != null)
				{
					if (VampNoteOnsets.Timings.Markers.Count > 0)
					{
						...
						if (!allInOne)
						{
							writer = Begin_xTimingsFile(fileBaseName + " - " + VampNoteOnsets.Timings.Name + exten);
							writeCount++;
						}
			...
			if (chkPolyphonic.Checked)
			{
				if (VampPolyphonic.Timings != null)
				{
					if (VampPolyphonic.Timings.Markers.Count > 0)
					{
						...
						if (!allInOne)
						{
							writer = Begin_xTimingsFile(fileBaseName + " - " + VampPolyphonic.Timings.Name + exten);
							writeCount++;
						}
			...
			if (chkPitchKey.Checked)
			{
				if (VampPitchKey.Timings != null)
				{
					if (VampPitchKey.Timings.Markers.Count > 0)
					{
						...
						if (!allInOne)
						{
							writer = Begin_xTimingsFile(fileBaseName + " - " + VampPitchKey.Timings.Name + exten);
							writeCount++;
						}
			...
			if (chkTempo.Checked)
			{
				if (VampTempo.Timings != null)
				{
					if (VampTempo.Timings.Markers.Count > 0)
					{
						...
						if (!allInOne)
						{
							writer = Begin_xTimingsFile(fileBaseName + " - " + VampTempo.Timings.Name + exten);
							writeCount++;
						}
			...
			if (chkSegments.Checked)
			{
				if (VampSegments.Timings != null)
				{
					if (VampSegments.Timings.Markers.Count > 0)
					{
						...
						if (!allInOne)
						{
							writer = Begin_xTimingsFile(fileBaseName + " - " + VampSegments.Timings.Name + exten);
							writeCount++;
						}
			...
			if (chkChromagram.Checked)
			{
				if (VampChromagram.Timings != null)
				{
					...
					if (VampChromagram.Timings.Markers.Count > 0)
					{
						...
						if (!allInOne)
						{
							writer = Begin_xTimingsFile(fileBaseName + " - " + VampChromagram.Timings.Name + exten);
							writeCount++;
						}
			...
			if (chkChords.Checked)
			{
				if (VampChords.Timings != null)
				{
					if (VampChords.Timings.Markers.Count > 0)
					{
						...
						if (!allInOne)
						{
							writer = Begin_xTimingsFile(fileBaseName + " - " + VampChords.Timings.Name + exten);
							writeCount++;
						}
		...
		{
			// Converts a formatted time (length) string to a numeric time in MILLIseconds
			int ret = 0;
			string rest = "";
			string hrs = "00";
			string min = "00";
		...
		{
			// Converts a TimeSpan object into a numerical length in MILLIseconds
		...
		public StreamWriter Begin_xTimingsFile(string fileName)
		{
			if (writer != null)
			{
				writer.Close();
		...
		public string xTimingsOut(xTimings timings, vamps.LabelType labelType, bool indent = false)
		{
			...
			for (int i = 0; i < timings.Markers.Count; i++)
			{
				...
				ret.Append(xTimings.RECORD_done);
				ret.Append(xTimings.CRLF);
			}
