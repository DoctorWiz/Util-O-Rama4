...
namespace UtilORama4
{
	public partial class frmVamp : Form
	{
		...
		private MRUoRama mruTimings = new MRUoRama(MRUoRama.FileType.Timings, "TimingFiles");
		private MRUoRama mruAudio = new MRUoRama(MRUoRama.FileType.Audio, "AudioFiles");
		...
		public static string applicationName = "Vamp-O-Rama";  // Or changed to "Vamperizer" if only xLights is installed (but not Light-O-Rama)
		...
		private void InitForm()
		{
			...
			//mruTimings.Validate();
			string tim0 = mruTimings.GetFile(0);
			if (tim0.Length < 5)
			{
		...
		{
			mruAudio.FillFileComboBox(cboFile_Audio);
		}
		...
		private void SaveUserSettings()
		{
			...
			mruAudio.UseFile(fileAudioLast);
...
*/
		private void btnSavexL_Click(object sender, EventArgs e)
		{
			...
			string initDir = userSettings.LastxLightsTimingsSavePath;
			...
			if (result == DialogResult.OK)
			{
				...
				if (ow == DialogResult.Yes)
				{
					...
					userSettings.LastxLightsTimingsSavePath = Path.GetDirectoryName(dlgFileSave.FileName);
					ExportSelectedVampsToxLights(dlgFileSave.FileName);
					mruTimings.UseFile(fileTimingsLast);
					dirtyTimes = false;
		...
		private void SaveTimings(object sender, EventArgs e)
		{
			...
			string idr = mruTimings.GetFile(0);
			bool saveAsk = true;
			string fileSeqSave = "!!!!!";
		...
		private void btnExport_Click(object sender, EventArgs e)
		{
			...
			pathExport = Path.GetDirectoryName(mruTimings.GetFile(0));
			...
			if (pathExport.Length < 4)
			{
				pathExport = Path.GetDirectoryName(mruAudio.GetFile(0));
				if (!Directory.Exists(pathExport))
				{
					// No longer exists, so reset to blank
					pathExport = "";
	...
	writer.WriteLine(xDat);
		...
		private void zzzOLD_SetAlignments()
		{
			...
			SetCombo(cboAlignOnsets, userSettings.alignBarBeats);
		}
