...
namespace LORUtils
{
	public partial class Sequence4 : IMember, IComparable<IMember>
	{
														...
														else // Not a regular channel
														{
															...
															else // not a track
															{
																...
																if (li > 0)
																{
																	...
																	if (lastGrid.gridType == TimingGridType.Freeform)
																	{
																		lineIn = reader.ReadLine();
																		lineCount++;
			...
			if (!theGroup.Written)
			{
				if ((!selectedOnly) || (theGroup.Selected))
				{
						...
						(itemTypes == MemberType.ChannelGroup))
					{
						...
						lineOut += utils.FIELDname + utils.FIELDEQ + utils.dirtyName(theGroup.Name) + utils.ENDQT;
		...
		private int WriteTrack(Track theTrack, bool selectedOnly, MemberType itemTypes)
		{
			...
			{
				lineOut += utils.FIELDname + utils.FIELDEQ + utils.dirtyName(theTrack.Name) + utils.ENDQT;
			}
			lineOut += utils.FINFLD;
...
//									if (item.Name.IndexOf("lyphonic") > 0) System.Diagnostics.Debugger.Break(); // Break here for debugging if named group is found
		...
		public int[] DuplicateNameCheck()
		{
			...
			if (Channels.Count > 0)
			{
				...
				for (int ch = 0; ch < Channels.Count; ch++)
				{
					...
					matches[ch].type = MemberType.Channel;
			...
			if (RGBchannels.Count > 0)
			{
				...
				for (int rg = 0; rg < RGBchannels.Count; rg++)
				{
					...
					matches[q].type = MemberType.RGBchannel;
			...
			if (ChannelGroups.Count > 0)
			{
				for (int gr = 0; gr < ChannelGroups.Count; gr++)
				{
					...
					matches[q].type = MemberType.ChannelGroup;
		...
		public IMember Duplicate()
		{
			...
			for (int idx = 0; idx < Members.bySavedIndex.Count; idx++)
			{
				...
				{
					Channel newCh = (Channel)Members.bySavedIndex[idx].Duplicate();
