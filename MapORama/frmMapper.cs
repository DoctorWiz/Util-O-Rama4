...
namespace UtilORama4
{
	public partial class frmMapper : Form
	{
		...
		private int noSizeX = 0;
		private int noSizeY = 0;

		public const string title = "Map-O-Rama";
		...
		private void frmMapper_ResizeEnd(object sender, EventArgs e)
		{
			const int sideGaps = 16;
			const int centerGap = 116;
			int midButtonGap = (centerGap - btnMap.Width) / 2;
			const int vGapBig = 10;
			const int vGapSmall = 4;
			const int vGapTiny = 2;
			int ctlBorders = treeSource.Width - treeSource.ClientRectangle.Width;
			int ctlBorder = ctlBorders / 2;
			// Temps
			int iw = 0;
			int ix = 0;
			int iy = 0;
			...
			if (noSizeX < 1)
			{
				// Left edge of Source File stuff
				//   These items never move horizontally
				lblSourceFile.Left = sideGaps;
				cboSourceFile.Left = sideGaps;
				lblSourceTree.Left = sideGaps;
				picPreviewSource.Left = sideGaps + ctlBorder;
				lblMessage.Left = sideGaps;
				chkCopyBeats.Left = sideGaps;
				lblDestAlreadyMapped.Width = centerGap;
				lblDestHasEffects.Width = centerGap;
				lblMapped.Width = centerGap;

				// Between left border and source tree
				noSizeX += sideGaps;
				// Between source and destination trees
				noSizeX += centerGap;
				// Between destination tree and right border
				noSizeX += sideGaps;
				// Borders
				noSizeX += this.Width - this.ClientRectangle.Width;
			}

			// *** Y ***
			if (noSizeY < 1)
			{
				// Between Menu Bar and file box labels
				noSizeY += vGapSmall;
				// File Box Labels
				//    Never moves vertically
				lblSourceFile.Top = noSizeY;
				lblDestFile.Top = noSizeY;
				lblMaster.Top = noSizeY;
				noSizeY += lblSourceFile.Height;
				// Between file box labels and File ComboBoxes
				noSizeY += vGapSmall;
				// File ComboBox
				//   Never moves vertically
				cboSourceFile.Top= noSizeY;
				btnBrowseSource.Top = noSizeY;
				cboDestFile.Top = noSizeY;
				btnBrowseDest.Top = noSizeY;
				noSizeY += cboSourceFile.Height;
				// Between File ComboBoxes and Tree Copy Label
				noSizeY += vGapSmall;
				// Tree Copy Labels
				lblSourceTree.Left= sideGaps;
				lblSourceTree.Top= noSizeY;
				lblDestTree.Top = noSizeY;
				noSizeY += lblSourceTree.Height;
				// Between Tree Copy Labels and the Trees
				//   Tree Never Changes position (although it does resize)
				noSizeY += vGapSmall;
				treeSource.Left= sideGaps;
				treeSource.Top= noSizeY;
				treeDest.Top = noSizeY;
				btnAutoMap.Top= noSizeY;
				// Between Trees and Preview
				noSizeY += vGapSmall;
				// Preview Windows
				//    All the controls from here down will move up/down according to form size
				noSizeY += picPreviewSource.Height;
				// Between preview windows and preview labels
				noSizeY += vGapTiny;
				// Preview Labelws
				noSizeY += lblSourceMappedTo.Height;
				// Between preview labels and load mapping button
				noSizeY += vGapSmall;
				// Load and Save Mapping buttons, and mapping file combo
				noSizeY += btnLoadMap.Height;
				// Between Load/Save buttons and Make New button
				noSizeY += vGapSmall;
				// Make New Button
				noSizeY += btnSaveNewSeq.Height;
				// Between Make New button and status bar
				noSizeY+= vGapTiny;
				// Status Bar
				noSizeY += staStatus.Height;
				// Borders
				noSizeY += this.Height- this.ClientRectangle.Height;
				// Menu Bar
				noSizeY += mnuFile.Height;

			}

			// Only part that is really sizeable is the TreeViews, File ComboBoxes, and Previews
			// But once resized, lottsa stuff needs to be repositioned...
			// Width- Trees
			pnlAll.Width = this.Width;
			treeSource.Width= (this.Width - noSizeX)/2;
			treeDest.Width = (this.Width - noSizeX) - treeSource.Width; // Do it this way in case noSizeX is an odd number
			// Width- Source Stuff
			// Width- File ComboBoxes
			btnBrowseSource.Left = treeSource.Left + treeSource.Width - btnBrowseSource.Width;
			cboSourceFile.Width = treeSource.Width - btnBrowseSource.Width - vGapSmall;
			// Width- Previews
			picPreviewSource.Width = treeSource.Width; // - ctlBorders;
			lblSourceMappedTo.Width = treeSource.Width;
			// Width- Destination Stuff
			// Width- File ComboBoxes
			cboDestFile.Width = treeDest.Width - btnBrowseDest.Width - vGapSmall;
			// Width- Previews
			pnlOverwrite.Width = treeDest.Width;
			picPreviewDest.Width = treeDest.Width - ctlBorders;
			lblDestMappedTo.Width = treeDest.Width;

			// Set left edge of all 'Destination' sequence stuff
			//   Note: Left edge of 'Source' controls are set above
			ix = treeSource.Left + treeSource.Width + centerGap;
			lblDestFile.Left = ix;
			lblMaster.Left = lblDestFile.Left + lblDestFile.Width + vGapBig;
			cboDestFile.Left = ix;
			btnBrowseDest.Left = cboDestFile.Left + cboDestFile.Width + vGapSmall;
			lblDestTree.Left = ix;
			treeDest.Left = ix;
			pnlOverwrite.Left = ix;
			picPreviewDest.Left = ix + ctlBorder;
			lblDestMappedTo.Left = ix;

			// Set the tops of all the center stuff
			iy = treeSource.Top;
			pnlMapWarn.Top = iy;
			iy += ctlBorder;
			btnMap.Top = iy;
			iy += ctlBorder + vGapSmall;
			lblDestHasEffects.Top = iy;
			iy += lblDestHasEffects.Height + vGapSmall;
			lblDestAlreadyMapped.Top = iy;
			iy += lblDestAlreadyMapped.Height + vGapSmall;
			btnUnmap.Top = iy;
			iy += btnUnmap.Height + vGapBig;
			btnAutoMap.Top = iy;
			iy = btnAutoMap.Top + btnUnmap.Height + vGapSmall;
			chkFuzzy.Top= iy;
			iy += chkFuzzy.Height + vGapBig;
			btnSummary.Top = iy;
			iy += btnSummary.Height + vGapBig;
			picAboutIcon.Top = iy;
			iy += picAboutIcon.Height + vGapBig;
			lblMapped.Top = iy;
			
			// Set left edge of all the center stuff
			ix = treeSource.Left + treeSource.Width;
			lblDestHasEffects.Left = ix;
			lblDestAlreadyMapped.Left = ix;
			lblMapped.Left = ix;
			iw = (centerGap - chkFuzzy.Width) / 2 + ctlBorder;
			chkFuzzy.Left = ix + iw;
			// Note: Since the icon is wider than the gap, this will result in a negative offset
			iw = (centerGap - picAboutIcon.Width)/2;
			picAboutIcon.Left = ix+ iw;
			ix += midButtonGap;
			btnAutoMap.Left = ix;
			pnlMapWarn.Left = ix - ctlBorder;
			btnMap.Left= ix;
			btnUnmap.Left = ix;
			btnSummary.Left = ix;
			chkRamps.Left = ix;

			// Only controls to change height are the TreeViews
			treeSource.Height = this.ClientRectangle.Height - noSizeY;
			treeDest.Height = treeSource.Height;
			
			// Set top of all controls below the TreeViews
			iy = treeSource.Top + treeSource.Height + vGapTiny;
			picPreviewSource.Top = iy;
			chkRamps.Top = iy;
			picPreviewDest.Top = iy;
			iy += picPreviewSource.Height;
			lblSourceMappedTo.Top = iy;
			lblDestMappedTo.Top = iy;
			iy += lblSourceMappedTo.Height + vGapSmall;
			pnlMessage.Top = iy;
			lblMessage.Top = iy + ctlBorder;
			iy += lblMessage.Height + vGapBig;
			btnLoadMap.Top = iy;
			btnSaveMap.Top = iy;
			cboMappingFile.Top = iy + vGapTiny;
			iy += btnLoadMap.Height + vGapSmall;
			btnSaveNewSeq.Top = iy;
			chkCopyBeats.Top = iy + vGapSmall;
			iy += btnSaveMap.Height;
			iy += vGapBig;
			pnlAll.Height= iy;

			// Set left edge of remaining stragglers
			iw = btnLoadMap.Width + vGapSmall + cboMappingFile.Width + vGapSmall + btnSaveMap.Width;
			ix = (ClientRectangle.Width - iw) / 2;
			btnLoadMap.Left = ix;
			ix += btnLoadMap.Width + vGapSmall;
			cboMappingFile.Left = ix;
			ix += cboMappingFile.Width + vGapSmall;
			btnSaveMap.Left = ix;

			// Last, Final thing (I hope) is 'Make New' button & AutoLaunch
			ix = (this.ClientRectangle.Width - btnSaveNewSeq.Width) / 2;
			btnSaveNewSeq.Left= ix;
			chkAutoLaunch.Left = btnSaveNewSeq.Left + btnSaveNewSeq.Width + vGapBig;
			chkAutoLaunch.Top = btnSaveNewSeq.Top + btnSaveNewSeq.Height - chkAutoLaunch.Height;



		}

		private void pnlOverwrite_Paint(object sender, PaintEventArgs e)
		{
