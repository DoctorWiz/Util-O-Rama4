			...
			for (int t = 0; t < seq.Tracks.Count; t++)
			{
				...
				for (int ti = 0; ti < theTrack.Children.Count; ti++)
				{
					...
					if (part != null)
					{
						if (part.TableType == TableType.ChannelGroup)
						{
							c += BuildGroup(seq, (ChannelGroup)part, level+1, ref count, ref savedIndexes, ref levels, selectedOnly, includeRGBchildren);
						}
						if (part.TableType == TableType.RGBchannel)
						{
							c += BuildRGBchannel(seq, (RGBchannel)part, level + 1, ref count, ref savedIndexes, ref levels, selectedOnly, includeRGBchildren);
						}
						if (part.TableType == TableType.Channel)
						{
							c += BuildChannel(seq, (Channel)part, level + 1, ref count, ref savedIndexes, ref levels, selectedOnly);
						}
					} // end not null
						//} // end try
					#region catch1
