	...
	public partial class Sequence4 : SeqPart, IComparable<SeqPart>
	{
		...
		public int WriteSequenceFile_DisplayOrder(string newFileName, bool selectedOnly, bool noEffects)
		{
			...
			foreach (Track theTrack in Tracks)
			{
				...
				{
					int tgsi = theTrack.timingGrid.SaveID;
					if (tgsi < 0)
					{
					...
					if (TimingGrids[tgsi].AltSaveID == utils.UNDEFINED)
					{
						int asi = Children.GetNextAltSaveID(TimingGrids[tgsi]);
						//TimingGrids[tgsi].AltSavedIndex = asi;
						//altSaveIDs[tgoi] = altSaveID;
						// If the track is Selected, it's timingGrid is also supposed to be Selected
						TimingGrids[tgsi].Selected = true;
					}
				}
			}
			for (int tg = 0; tg < TimingGrids.Count; tg++)
		...
		private int WriteChannelGroup(ChannelGroup theGroup, bool selectedOnly, bool noEffects, TableType itemTypes)
		{
			if (!theGroup.Written)
			{
				if ((!selectedOnly) || (theGroup.Selected))
				{
					...
					{
						WriteItems(theGroup.Children, selectedOnly, noEffects, itemTypes);
					}
