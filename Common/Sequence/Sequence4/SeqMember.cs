...
namespace LORUtils
{
	...
	public class TimingGrid : IMember, IComparable<IMember>
	{
		...
		public TimingGridType gridType = TimingGridType.None;
		...
		public string LineOut()
		{
			...
			ret += utils.FIELDtype + utils.FIELDEQ + SeqEnums.TimingName(gridType) + utils.ENDQT;
			if (spacing > 1)
			{
				ret += FIELDspacing + utils.FIELDEQ + spacing.ToString() + utils.ENDQT;
			}
			if (gridType == TimingGridType.FixedGrid)
			{
				ret += utils.ENDFLD;
			}
			else if (gridType == TimingGridType.Freeform)
			{
				ret += utils.FINFLD;
		...
		public void Parse(string lineIn)
		{
			...
			gridType = SeqEnums.enumGridType(utils.getKeyWord(lineIn, utils.FIELDtype));
		...
		public IMember Duplicate()
		{
			...
			grid.gridType = this.gridType;
			grid.spacing = spacing;
			if (this.gridType == TimingGridType.Freeform)
			{
