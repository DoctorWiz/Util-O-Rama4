...
namespace LORUtils4
{
	public class lutils
	{
		...
		public static void FillChannels(TreeView tree, LORSequence4 seq, List<List<TreeNode>> siNodes, bool selectedOnly, bool includeRGBchildren)
		{
		...
		public static void FillChannels(TreeView tree, LORSequence4 seq, List<List<TreeNode>> siNodes, bool selectedOnly, bool includeRGBchildren, int memberTypes)
		{
		...
		private static TreeNode AddTrack(LORSequence4 seq, TreeNodeCollection baseNodes, int trackNumber, List<List<TreeNode>> siNodes, bool selectedOnly, bool includeRGBchildren, int memberTypes)
		{
			...
			for (int ti = 0; ti < theTrack.Members.Count; ti++)
			{
				//try
				//{
				LORMember4 member = theTrack.Members.Items[ti];
		...
		private static TreeNode AddGroup(LORSequence4 seq, TreeNodeCollection baseNodes, int groupIndex, List<List<TreeNode>> siNodes, bool selectedOnly, bool includeRGBchildren, int memberTypes)
		{
			//ChanInfo nodeTag = new ChanInfo();
			//nodeTag.LORMemberType4 = LORMemberType4.LORChannelGroup4;
			...
			for (int gi = 0; gi < theGroup.Members.Count; gi++)
			{
				//try
				//{
				LORMember4 member = theGroup.Members.Items[gi];
				int si = member.SavedIndex;
				if (member.LORMemberType4 == LORMemberType4.LORChannelGroup4)
				{
					LORChannelGroup4 memGrp = (LORChannelGroup4)member;
		...
		*/
		...
		private static TreeNode AddChannel(LORSequence4 seq, TreeNodeCollection baseNodes, int channelIndex, bool selectedOnly)
		{
			string nodeText = seq.Channels[channelIndex].Name;
			TreeNode channelNode = baseNodes.Add(nodeText);
			LORChannel4 theChannel = seq.Channels[channelIndex];
			//LORMember4 nodeTag = theChannel;
			nodeIndex++;
		...
		private static TreeNode AddRGBchannel(LORSequence4 seq, TreeNodeCollection baseNodes, int RGBIndex, List<List<TreeNode>> siNodes, bool selectedOnly, bool includeRGBchildren)
		{
			List<TreeNode> qlist;

			string nodeText = seq.RGBchannels[RGBIndex].Name;
			TreeNode channelNode = baseNodes.Add(nodeText);
			LORRGBChannel4 theRGB = seq.RGBchannels[RGBIndex];
			//LORMember4 nodeTag = theRGB;
			nodeIndex++;
		...
		*/
		...
		public static int BuildDisplayOrder(LORSequence4 seq, ref int[] savedIndexes, ref int[] levels, bool selectedOnly, bool includeRGBchildren)
		{
			//TODO: 'Selected' not implemented yet
			...
			for (int t = 0; t < seq.Tracks.Count; t++)
			{
				...
				for (int ti = 0; ti < theTrack.Members.Count; ti++)
				{
					...
					if (member != null)
					{
						if (member.LORMemberType4 == LORMemberType4.LORChannelGroup4)
						{
							c += BuildGroup(seq, (LORChannelGroup4)member, level+1, ref count, ref savedIndexes, ref levels, selectedOnly, includeRGBchildren);
						}
						if (member.LORMemberType4 == LORMemberType4.LORRGBChannel4)
						{
							c += BuildRGBchannel(seq, (LORRGBChannel4)member, level + 1, ref count, ref savedIndexes, ref levels, selectedOnly, includeRGBchildren);
						}
						if (member.LORMemberType4 == LORMemberType4.LORChannel4)
						{
							c += BuildChannel(seq, (LORChannel4)member, level + 1, ref count, ref savedIndexes, ref levels, selectedOnly);
						}
					} // end not null
						//} // end try
					#region catch1
		...
		public static int BuildGroup(LORSequence4 seq, LORChannelGroup4 theGroup, int level, ref int count, ref int[] savedIndexes, ref int[]levels, bool selectedOnly, bool includeRGBchildren)
		{
			int c = 0;
			string nodeText = theGroup.Name;
		...
		*/
		...
		public static int BuildChannel(LORSequence4 seq, LORChannel4 theChannel, int level, ref int count, ref int[] savedIndexes, ref int[] levels, bool selectedOnly)
		{
			int c = 0;
		...
		public static int BuildRGBchannel(LORSequence4 seq, LORRGBChannel4 theRGB, int level, ref int count, ref int[] savedIndexes, ref int[] levels, bool selectedOnly, bool includeRGBchildren)
		{
			int c = 0;
		...
		public static Bitmap RenderEffects(LORMember4 member, int startCentiseconds, int endCentiseconds, int width, int height, bool useRamps)
		{
			// Create a temporary working bitmap
					...
					while (keepGoing)
					{
						...
						if (thisClik >= curEffect.startCentisecond)
						{
							...
							if (thisClik <= curEffect.endCentisecond)
							{
								...
								if (curEffect.LOREffectType4 == LOREffectType4.intensity)
								{
									curLevel = curEffect.Intensity;
								}
								if (curEffect.LOREffectType4 == LOREffectType4.shimmer)
								{
									curLevel = curEffect.Intensity | ADDshimmer;
								}
								if (curEffect.LOREffectType4 == LOREffectType4.twinkle)
								{
