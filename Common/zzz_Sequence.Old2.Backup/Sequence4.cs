...
namespace LORUtils4
{
	public partial class LORSequence4 : LORMember4, IComparable<LORMember4>
	{
		...
		public LORAnimation animation = null;
		...
		private struct Match
		{
			public string Name;
			public int savedIdx;
			public LORUtils4.LORMemberType4 LORMemberType4;
			public int itemIdx;
		}



		#region LORMember4 Interface
		public string Name
		{
			get
			{
		...
		{
			string ret = "";
			// Implemented primarily for compatibility with 'LORMember4' interface
		...
		public LORSequence4()
		{
			...
			this.animation = new LORAnimation(this);
																		...
																		else // not a loop
																		{
																			...
																			else
																			{
																				...
																				else
																				{
																					...
																					if (li > 0)
																					{
																						animation = new LORAnimation(this, lineIn);
																					} // end if LORAnimation, or not
																				} // end if LORAnimationColumn4, or not
																			} // end if LORAnimation, or not
																		} // end if LORLoopLevel4, or not
																	} // end if LORLoop4, or not (as in a loopLevel loop, not a for loop)
																} // end LORTimings4 (or not)
															} // end LORTrack4 (or not)
														} // end LORTrack4 Items (or not)
													} // end LORChannelGroup4 (or not)
												} // end LORRGBChannel4 (or not)
											} // end regular LORChannel4 (or not)
										} // end timing (or not)
									} // end LOREffect4 (or not)
										/*
							} // end 2nd Try
									catch (Exception ex)
									{
		...
		private int WriteRGBchannel(LORRGBChannel4 theRGBchannel, bool selectedOnly, bool noEffects, LORMemberType4 itemTypes)
		{
			if (!theRGBchannel.Written)
			{
				...
				{
					if ((itemTypes == LORMemberType4.Items) || (itemTypes == LORMemberType4.Items) || (itemTypes == LORMemberType4.LORChannel4))
					{
						//theRGBchannel.LineOut(selectedOnly, noEffects, LORMemberType4.LORChannel4);
					...
					if ((itemTypes == LORMemberType4.Items) || (itemTypes == LORMemberType4.Items) || (itemTypes == LORMemberType4.LORRGBChannel4))
					{
						...
						lineOut = theRGBchannel.LineOut(selectedOnly, noEffects, LORMemberType4.LORRGBChannel4);
						writer.WriteLine(lineOut);
			...
			if (!theGroup.Written)
			{
				if ((!selectedOnly) || (theGroup.Selected))
				{
					if ((itemTypes == LORMemberType4.Items) ||
						(itemTypes == LORMemberType4.LORChannel4) ||
						(itemTypes == LORMemberType4.LORRGBChannel4))
					{
						// WriteListItems may in turn call this procedure (WriteChannelGroup) for any Child LORChannel4 Groups
						// Therefore making it recursive at this point
						altSIs = WriteListItems(theGroup.Members, selectedOnly, noEffects, itemTypes);
					}

					if ((itemTypes == LORMemberType4.Items) ||
						(itemTypes == LORMemberType4.LORChannelGroup4))
					{
						...
						lineOut += lutils.FIELDname + lutils.FIELDEQ + lutils.dirtyName(theGroup.Name) + lutils.ENDQT;
		...
		private int WriteTrack(LORTrack4 theTrack, bool selectedOnly, LORMemberType4 itemTypes)
		{
			...
			int altID = theTrack.timingGrid.AltSaveID;
			...
			{
				lineOut += lutils.FIELDname + lutils.FIELDEQ + lutils.dirtyName(theTrack.Name) + lutils.ENDQT;
			}
			lineOut += lutils.FINFLD;
		...
		private List<int> WriteListItems(LORMembership itemIDs, bool selectedOnly, bool noEffects, LORMemberType4 itemTypes)
		{
			...
			foreach (LORMember4 item in itemIDs.Items)
			{
				...
				if ((!selectedOnly) || (item.Selected))
				{
					if (!item.Written)
					{
						if (item.LORMemberType4 == LORMemberType4.LORChannel4)
						{
						...
						{
							if (item.LORMemberType4 == LORMemberType4.LORRGBChannel4)
							{
								LORRGBChannel4 theRGB = (LORRGBChannel4)item;
								if ((itemTypes == LORMemberType4.Items) || (itemTypes == LORMemberType4.Items) || (itemTypes == LORMemberType4.LORChannel4) || (itemTypes == LORMemberType4.LORRGBChannel4))
								{
							...
							{
								if (item.LORMemberType4 == LORMemberType4.LORChannelGroup4)
								{
...
//									if (item.Name.IndexOf("lyphonic") > 0) System.Diagnostics.Debugger.Break(); // Break here for debugging if named group is found
		...
		private int WriteItemsList(LORMembership itemIDs, bool selectedOnly, LORMemberType4 itemTypes)
		{
			...
			if (itemIDs.owner.LORMemberType4 == LORMemberType4.LORTrack4)
			{
				lineOut += lutils.TABLEchannel;
				leader += lutils.TABLEchannel;
			}
			if (itemIDs.owner.LORMemberType4 == LORMemberType4.LORChannelGroup4)
			{
			...
			foreach (LORMember4 subID in itemIDs.Items)
			{
				...
				if (subID.Written)  // The item itself should have already been written!
				{
					...
					if (!selectedOnly || sel)
					{
						if ((itemTypes == subID.LORMemberType4) || (itemTypes == LORMemberType4.Items))
						{
		...
		public int WriteItem(int SavedIndex, bool selectedOnly, bool noEffects, LORMemberType4 theType)
		{
			...
			LORMember4 member = Members.bySavedIndex[SavedIndex];
			if (!member.Written)
			{
				if (!selectedOnly || member.Selected)
				{
					LORMemberType4 itemType = member.LORMemberType4;
					if (itemType == LORMemberType4.LORChannel4)
					{
						LORChannel4 theChannel = (LORChannel4)member;
						if ((theType == LORMemberType4.LORChannel4) || (theType == LORMemberType4.Items))
						{
					...
					{
						if (itemType == LORMemberType4.LORRGBChannel4)
						{
						...
						{
							if (itemType == LORMemberType4.LORChannelGroup4)
							{
		...
		private void WriteSequenceClose()
		{
			...
			// Write out LORAnimation info, it it exists
			WriteAnimation();
		...
		public string summary()
		{
			...
			sMsg += "     LORChannel4 Groups: " + ChannelGroups.Count.ToString() + lutils.CRLF;
			sMsg += "       Timing Grids: " + TimingGrids.Count.ToString() + lutils.CRLF;
			sMsg += "             Tracks: " + Tracks.Count.ToString() + lutils.CRLF;
			sMsg += "       Centiseconds: " + myCentiseconds.ToString() + lutils.CRLF;
			sMsg += "Highest Saved Index: " + Members.HighestSavedIndex.ToString() + lutils.CRLF;
			return sMsg;
		}
		...
		public int[] DuplicateNameCheck()
		{
			...
			if (Channels.Count > 0)
			{
				...
				for (int ch = 0; ch < Channels.Count; ch++)
				{
					...
					matches[ch].LORMemberType4 = LORMemberType4.LORChannel4;
			...
			if (RGBchannels.Count > 0)
			{
				...
				for (int rg = 0; rg < RGBchannels.Count; rg++)
				{
					...
					matches[q].LORMemberType4 = LORMemberType4.LORRGBChannel4;
					matches[q].itemIdx = rg;

					q++;
				}
			} // RGB LORChannel4 Count > 0
			...
			if (ChannelGroups.Count > 0)
			{
				for (int gr = 0; gr < ChannelGroups.Count; gr++)
				{
					...
					matches[q].LORMemberType4 = LORMemberType4.LORChannelGroup4;
		...
		public int ReadClipboardFile(string existingFilename)
		{
			...
			while ((lineIn = reader.ReadLine()) != null)
			{
				...
				if (pos1 > 0)
				{
					...
					//chan.deviceType = LORSeqEnums4.LOREnumDevice4(lutils.getKeyWord(lineIn, FIELDdeviceType));
				...
				if (pos1 > 0)
				{
					...
					ef.LOREffectType4 = LORSeqEnums4.LOREnumEffectType4(lutils.getKeyWord(lineIn, lutils.FIELDtype));
				...
				if (pos1 > 0)
				{
					...
					//tg.type = LORSeqEnums4.LOREnumGridType4(lutils.getKeyWord(lineIn, lutils.FIELDtype));
		...
		public LORChannel4 FindChannel(string channelName)
		{
			LORChannel4 ret = null;
			LORMember4 member = Members.Find(channelName, LORMemberType4.LORChannel4, false);
			if (member != null)
			{
				ret = (LORChannel4)member;
			}
		...
		{
			LORRGBChannel4 ret = null;
			LORMember4 member = Members.Find(rgbChannelName, LORMemberType4.LORRGBChannel4, false);
			if (member != null)
			{
				ret = (LORRGBChannel4)member;
			}
		...
		public LORChannelGroup4 FindChannelGroup(string channelGroupName)
		{
			LORChannelGroup4 ret = null;
			LORMember4 member = Members.Find(channelGroupName, LORMemberType4.LORChannelGroup4, false);
			if (member != null)
			{
				ret = (LORChannelGroup4)member;
			}
		...
		public LORTrack4 FindTrack(string trackName)
		{
			LORTrack4 ret = null;
			LORMember4 member = Members.Find(trackName, LORMemberType4.LORTrack4, false);
			if (member != null)
			{
		...
		public LORTimings4 FindTimingGrid(string timingGridName)
		{
			LORTimings4 ret = null;
			LORMember4 member = Members.Find(timingGridName, LORMemberType4.LORTimings4, false);
			if (member != null)
			{
				ret = (LORTimings4)member;
			}
			return ret;
		}
		#endregion
		...
		public int SavedIndexIntegrityCheck()
		{
			...
			for (int ch = 0; ch < Channels.Count; ch++)
			{
				if (Channels[ch].SavedIndex < tot)
				{
					...
					if (siCheck[Channels[ch].SavedIndex].LORMemberType4 == LORMemberType4.None)
					{
						siCheck[Channels[ch].SavedIndex].LORMemberType4 = LORMemberType4.LORChannel4;
					}
					else
					{
						errs++;
			...
			for (int rch = 0; rch < RGBchannels.Count; rch++)
			{
				if (RGBchannels[rch].SavedIndex < tot)
				{
					...
					if (siCheck[RGBchannels[rch].SavedIndex].LORMemberType4 == LORMemberType4.None)
					{
						siCheck[RGBchannels[rch].SavedIndex].LORMemberType4 = LORMemberType4.LORRGBChannel4;
					}
					else
					{
						errs++;
			...
			for (int chg = 0; chg < ChannelGroups.Count; chg++)
			{
				if (ChannelGroups[chg].SavedIndex < tot)
				{
					...
					if (siCheck[ChannelGroups[chg].SavedIndex].LORMemberType4 == LORMemberType4.None)
					{
						siCheck[ChannelGroups[chg].SavedIndex].LORMemberType4 = LORMemberType4.LORChannelGroup4;
					}
					else
					{
						errs++;
		...
		public int[] DuplicateChannelGroupNames()
		{
			...
			if (ChannelGroups.Count > 1)
			{
				...
				// return any matches found
			return ret;
		} // end Duplicate LORChannel4 Group Names
		...
		public int[] DuplicateRGBchannelNames()
		{
			...
			if (RGBchannels.Count > 1)
			{
				...
				// return any matches found
			return ret;
		} // end Duplicate RGB LORChannel4 Names
		...
		public int[] DuplicateNames()
		{
			...
			if (nameCount > 1)
			{
				...
				} // end LORChannel4 Group LORLoop4
		...
		public LORTrack4 ParseTrack(string lineIn)
		{
			...
			if (myAltSavedIndex > lutils.UNDEFINED)
			{
				...
				LORMember4 tg = Members.bySaveID[tr.AltSavedIndex];
				// And assign it to the track
				tr.timingGrid = (LORTimings4)tg;
				// Clear the AltSavedIndex which was temoorarily holding the SaveID of the LORTimings4
				tr.AltSavedIndex = lutils.UNDEFINED;
			}
		...
		private int AssignNextSavedIndex(LORMember4 thePart)
		{
		...
		private int AssignNextAltSavedIndex(LORMember4 thePart)
		{
		...
		{
			LORSequence4 seqOut = new LORSequence4();

			seqOut.animation = animation.Duplicate();
			...
			seqOut.LORSequenceType4 = sequenceType;
			seqOut.videoUsage = videoUsage;
			for (int idx = 0; idx < Members.bySavedIndex.Count; idx++)
			{
				LORMemberType4 mt = Members.bySavedIndex[idx].LORMemberType4;
				if (mt == LORMemberType4.LORChannel4)
				{
					LORChannel4 newCh = (LORChannel4)Members.bySavedIndex[idx].Duplicate();
