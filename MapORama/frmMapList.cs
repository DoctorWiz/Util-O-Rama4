	...
	public partial class frmMapList : Form
	{
		...
		public void FillBySourceAlpha()
		{
			...
			foreach (iLOR4Member sourceMember in seqSource.AllMembers.ByName.Values)
			{
				...
				LOR4RGBChild rchild = LOR4RGBChild.None;
		...
		{
			//TODO CheckState / Selected needs review!!
			level = 0; // Reset
								 // Add all mapped channels, dig thru source tree
			for (int t = 0; t < seqSource.Tracks.Count; t++)
			{
				// Add mapped channels
				BuildListSourceDisplayOrder(seqSource.Tracks[t], CheckState.Checked);  //? Correct??
			}
			...
			if (chkUnmapped.Checked)
			{
				...
				for (int t = 0; t < seqDest.Tracks.Count; t++)
				{
					...
					BuildListDestDisplayOrder(seqDest.Tracks[t], CheckState.Unchecked); //? Correct??
				}
			}
		}

		public void BuildListSourceDisplayOrder(iLOR4Member sourceMember, CheckState selected)
		{
			//TODO This needs work!
			if (sourceMember.SelectedState != selected) //? Correct??
			{
				if (selected == CheckState.Checked) //? Correct??
				{
				...
				switch (sourceMember.MemberType)
				{
					...
					case LOR4MemberType.Track:
						LOR4Track track = (LOR4Track)sourceMember;
		...
		public void BuildListDestDisplayOrder(iLOR4Member destMember, CheckState selected)
		{
			if (destMember.SelectedState == selected) //? Correct??
			{
				if (selected != CheckState.Checked) //? Correct??
				{
				...
				switch (destMember.MemberType)
				{
					...
					case LOR4MemberType.ChannelGroup:
						...
						break;
					case LOR4MemberType.Track:
						LOR4Track track = (LOR4Track)destMember;
						members = track.Members;
						break;
					case LOR4MemberType.Cosmic:
		...
		private void foo()
		{ 
			int c = LOR4Admin.DisplayOrderBuildLists(seqSource, ref savedIndexes, ref levels, false, false);
			...
			if (chkUnmapped.Checked)
			{
				c = LOR4Admin.DisplayOrderBuildLists(seqDest, ref savedIndexes, ref levels, false, false);
				for (int i = 0; i < savedIndexes.Length; i++)
				{
		...
		*/
		...
		public void FillByDestDispOrder()
		{
			...
			{
				// Add mapped channels
				BuildListDestDisplayOrder(seqDest.Tracks[t], CheckState.Checked); //? Correct??
			}
			// Also display Un-Mapped channels?
			if (chkUnmapped.Checked)
			{
				...
				for (int t = 0; t < seqSource.Tracks.Count; t++)
				{
					...
					BuildListSourceDisplayOrder(seqSource.Tracks[t], CheckState.Unchecked); //? Correct??
				}
			}
		...
		public void AddMapping(iLOR4Member sourceMember, iLOR4Member destMember, int indent)
		{
			...
			//string k = "";
			int[] k = null;
			int iconIndex = LOR4Admin.UNDEFINED;
			...
			if (destMember == null)
			{
				...
				{
					LOR4Channel ch = (LOR4Channel)sourceMember;
					iconIndex = LOR4Admin.ColorIcon(imlTreeIcons, ch.color);
				}
			}
			else
			{
