...
namespace LORUtils4
{
	public partial class LORSequence4 : LORMember4, IComparable<LORMember4>, IDisposable
	{
		...
		public List<LORCosmic> CosmicDevices = new List<LORCosmic>();
		...
		private LORMember4 mappedTo = null;
		...
		public LORMember4 Parent
		{
			get
			{
				if (isWiz)
				{
					...
					// This is a top level object, and has no parent (other than itself)
					System.Diagnostics.Debugger.Break();
				}
				return this;
			}
		...
		public void SetParent(LORMember4 newParent)
		{
			...
			if (isWiz)
			{
				// This is a top level object and has no parent
		...
		public LORMember4 MapTo
		{
			...
			set
			{
				if (value.MemberType == LORMemberType4.Sequence)
				{
					mappedTo = value;
				}
				else
		...
		//////////////////////////////
																	...
																	if (li < 0)
																	{
																		...
																		{
																			int isl = lutils.getKeyValue(lineIn, lutils.FIELDsavedIndex);
																			LORMember4 mm = Members.bySavedIndex[isl]
;																			lastCosmic.Members.Add(mm);
		...
		////////////////
																			...
																			if (li < 0)
																			{
																				...
																				while (li > 0)
																				{
																					...
																					{
																						LORMember4 SIMem = Members.bySavedIndex[isi];
																						if (SIMem != null)
																						{
		...
		private void ClearWrittenFlags()
		{
			...
			foreach (LORCosmic dev in CosmicDevices)
			{
				//chg.Written = false;
		...
		public void Clear(bool areYouReallySureYouWantToDoThis)
		{
			if (areYouReallySureYouWantToDoThis)
			{
				...
				CosmicDevices = new List<LORCosmic>();
		...
		public int ReadClipboardFile(string existingFilename)
		{
			...
			while ((lineIn = reader.ReadLine()) != null)
			{
				...
				if (pos1 > 0)
				{
					...
					ef.LOREffectType4 = LORSeqEnums4.EnumEffectType(lutils.getKeyWord(lineIn, lutils.FIELDtype));
		...
		{
			LORChannel4 ret = null;
			LORMember4 member = Members.Find(channelName, LORMemberType4.Channel, false);
			if (member != null)
			{
				ret = (LORChannel4)member;
			}
		...
		{
			LORRGBChannel4 ret = null;
			LORMember4 member = Members.Find(rgbChannelName, LORMemberType4.RGBChannel, false);
			if (member != null)
			{
				ret = (LORRGBChannel4)member;
			}
		...
		{
			LORChannelGroup4 ret = null;
			LORMember4 member = Members.Find(channelGroupName, LORMemberType4.ChannelGroup, false);
			if (member != null)
			{
				ret = (LORChannelGroup4)member;
			}
		...
		{
			LORTrack4 ret = null;
			LORMember4 member = Members.Find(trackName, LORMemberType4.Track, false);
			if (member != null)
			{
		...
		public LORTimings4 FindTimingGrid(string timingGridName, bool createIfNotFound = false)
		{
			...
			//LORMember4 member = Members.Find(timingGridName, LORMemberType4.Timings, createIfNotFound);
			//LORMember4 member = null;
			for (int i=0; i< TimingGrids.Count; i++)
			{
		...
		public LORCosmic ParseCosmicDevice(string lineIn)
		{
			LORCosmic cos = new LORCosmic("");
			cos.SetParent(this);
		...
		public LORCosmic CreateCosmicDevice(string theName)
		{
			...
			LORCosmic dev;
			dev = new LORCosmic(theName, lutils.UNDEFINED);
