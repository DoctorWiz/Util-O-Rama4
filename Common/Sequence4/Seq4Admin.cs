...
namespace LOR4
{
	public static class LOR4Admin
	{
		...
		*/
		...
		public static int ColorIcon(ImageList icons, Color color, int size=16)
		{
			int ret = -1;
			...
			if (ret < 0)
			{
				...
				Rectangle r = new Rectangle(2, 2, size-4, size-4);
				// Fill the middle rectangle with color
				gr.FillRectangle(b, r);
				// Draw a 3D button around it
				Pen p = new Pen(Color.White);
				gr.DrawLine(p, 0, 0, size - 1, 0);
				gr.DrawLine(p, 0, 1, 0, size - 1);
				p = new Pen(Color.Black);
				gr.DrawLine(p, 1, size-1, size-1, size-1);
				gr.DrawLine(p, size-1, 1, size-1, size-2);
				p = new Pen(Color.LightGray);
				gr.DrawLine(p, 1, 1, size - 2, 1);
				gr.DrawLine(p, 1, 2, 1, size - 2);
				p = new Pen(Color.DarkGray);
				gr.DrawLine(p, 2, size-2, size-2, size-2);
				gr.DrawLine(p, size-2, 2, size-2, size-3);
		...
		public static string ColorName(Color color)
		{
			if (Fyle.isWiz)
			{
				string sinfo = "The function ColorName is depreciated.  Use NearestColorName instead.";
				Debugger.Break();
			}
			return NearestColorName(color);
		}
		...
		public static string NearestColorName(Color color)
		{
			...
			if (color != null)
			{
				long cargb = color.ToArgb() & 0xFFFFFF; // Ignore alpha
																								// Special overrides for our custom "Common LED Colors", since they either won't match, or will override, any known color names.
				...
				{
					// Not a "Common LED Color", so find the nearest "Web" color name.
					int myR = color.R; int myG = color.G; int myB = color.B;
					Int32 diff = 9999999;
					foreach (KnownColor c in Enum.GetValues(typeof(KnownColor)))
					{
						Color kc = Color.FromKnownColor(c);
						// Exclude system colors, don't want things like "ActiveCaption" or "ControlText".
						if (!kc.IsSystemColor)
						{
							int cR = kc.R; int cG = kc.G; int cB = kc.B;
							int d = Math.Abs(myR - cR) + Math.Abs(myG - cG) + Math.Abs(myB - cB);
							if (d < diff)
							{
								diff = d;
								name = kc.Name;
