...
public class Channel : SeqPart, IComparable<SeqPart>
	{
		...
		private Sequence4 myParentSeq = null;
		...
		{
			int result = 0;
			if (myParentSeq.Children.sortMode == PartsCollection.SORTbySavedIndex)
			{
				result = mySavedIndex.CompareTo(other.SavedIndex);
			}
			else
			{
				if (myParentSeq.Children.sortMode == PartsCollection.SORTbyName)
				{
					result = myName.CompareTo(other.Name);
				}
				else
				{
					if (myParentSeq.Children.sortMode == PartsCollection.SORTbyAltSavedIndex)
					{
						result = myAltSavedIndex.CompareTo(other.AltSavedIndex);
					}
					else
					{
						if (myParentSeq.Children.sortMode == PartsCollection.SORTbyOutput)
						{
	...
	*/ 
	...
	public class ChannelGroup : SeqPart, IComparable<SeqPart>
	{
		...
		{
			SeqPart newPart = myParentSeq.Children.bySavedIndex[itemSavedIndex];
	...
	public class Effect
	{
		...
		public override string ToString()
		{
			...
			ret += " to " + endCentisecond.ToString();
			if (intensity > utils.UNDEFINED)
			{
	...
	public class Track : SeqPart, IComparable<SeqPart>
	{
		...
		public void Parse(string lineIn)
		{
			...
			else
			{
				...
				SeqPart part = myParentSeq.Children.bySaveID[tgsi];
	...
	{
		public int rows = utils.UNDEFINED;
		public int columns = utils.UNDEFINED;
		public string image = "";
	...
	*/
	...
	public class PartsCollection
	{
		...
		public Dictionary<string, SeqPart> byName = new Dictionary<string, SeqPart>();
		...
		public void ReIndex()
		{
			...
			byName = new Dictionary<string, SeqPart>();
