...
namespace UtilORama4
{
	partial class frmAbout : Form
	{
		...
		public string appName = "Something-O-Rama";
		public string appAbbr = "somethingorama";
		...
		public frmAbout(string appTitle)
		{ 
			InitializeComponent();
			bool alpha = false;
			bool beta = false;
			...
			if (xLightsInstalled && !lightORamaInstalled) // xLights Only
			{
				...
				picxLights.Left = lblBenefit.Left + lblBenefit.Width;
				picxLights.Top = lblBenefit.Top - 4;
				picLOR.Visible = false;
				lblAnd1.Visible = false;
				lblCommunity.Left = picxLights.Left + picxLights.Width;
				lblCommunity.Top = picxLights.Top + 4;
				...
				linkEmail.Text = "dev.xutils@wizster.com";
			}
			if (lightORamaInstalled && !xLightsInstalled) // Light-O-Rama Only
			{
				picxLights.Visible = false;
				picLOR.Left = lblBenefit.Left + lblBenefit.Width;
				picLOR.Top = lblBenefit.Top - 4;
				picLOR.Visible = true;
				lblAnd1.Visible = false;
				lblCommunity.Left = picLOR.Left + picLOR.Width;
				lblCommunity.Top = picLOR.Top + 4;
				...
				linkEmail.Text = "dev.utilorama@wizster.com";
			}
			if (lightORamaInstalled && !xLightsInstalled) // Both Light-O-Rama and xLights
			{
				...
				picLOR.Top = lblBenefit.Top - 4;
				picLOR.Visible = true;
				lblAnd1.Left = picLOR.Left + picLOR.Width;
				lblAnd1.Top = picLOR.Top + 4;
				picxLights.Left = lblAnd1.Left + lblAnd1.Width;
				picxLights.Visible = true;
				picxLights.Top = lblBenefit.Top - 4;
				lblCommunity.Left = picxLights.Left + picxLights.Width;
			...
			lblReleased.Top = lblProgram.Top;
			lblReleased.Left = lblProgram.Left + lblProgram.Width;
			lblReleased.Visible = true;
			linkCharity.Top = lblReleased.Top;
			linkCharity.Left = lblReleased.Left + lblReleased.Width;
			linkCharity.Visible = true;
			lblFor.Top = linkCharity.Top;
			lblFor.Left = linkCharity.Left + linkCharity.Width;
			lblFor.Visible = true;	

			this.Text = String.Format("About {0}", appName);
			this.lblProductName.Text = appName;
			...
			lblMember.Text = appName + lblMember.Text;
			...
			lblAnd1.Left = linkAuthorName.Left + linkAuthorName.Width;
			linkCompanyName.Left = lblAnd1.Left + lblAnd1.Width;
			lblDisclaimer.Width = btnOK.Left - lblDisclaimer.Left - 5;

			if (xLightsInstalled)
			{
				tipTool.SetToolTip(linkCharity, "https://www.paypal.com/donate/xLightsDevelopment");
			}
			else
			{
				tipTool.SetToolTip(linkCharity, "http://wizlights.com/utilorama/charitylist.html");
			}
		...
		private void labelUtils_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			...
			{
				OpenSite( "http://wizlights.com/xutils/");
			}
			else
			{
				OpenSite("http://wizlights.com/utilorama/");
			}
		}
		...
		private void picIcon_Click(object sender, EventArgs e)
		{
			if (xLightsInstalled && !lightORamaInstalled)
			{
				OpenSite("http://wizlights.com/xutils/" + appAbbr + "/");
			}
			else
			{
				OpenSite("http://wizlights.com/utilorama/" + appAbbr + "/");
			}
		}
		...
		private void lblEmail_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			if (xLightsInstalled && !lightORamaInstalled)
			{
				OpenSite("mailto:dev.xutils@wizlights.com");
			}
			else
			{
				OpenSite("mailto:dev.utilorama@wizlights.com");
			}
		}
		...
		{
			OpenSite("http://wizlights.com/utilorama/thanks.html");
		}

		private void lblGitIssues_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			OpenSite("https://github.com/DoctorWiz/Util-O-Rama4/issues");
		}
		...
		private void linkCharity_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			...
			if (xLightsInstalled)
			{
				OpenSite("https://www.paypal.com/donate/?hosted_button_id=BB6366BT755H6");
			}
			else
			{
				OpenSite("http://wizlights.com/utilorama/charitylist.html");
			}
		}
		private void picxLights_Click(object sender, EventArgs e)
		{
			OpenSite("http://xlights.org/");
		}
