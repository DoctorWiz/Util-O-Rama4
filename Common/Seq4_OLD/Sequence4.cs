...
namespace LOR4Utils
{
	public partial class LOR4Sequence : LOR4MemberBase, iLOR4Member, IComparable<iLOR4Member>, IDisposable
	{
		...
		private const string STARTsequence = lutils.STFLD + TABLEsequence + LOR4SequenceInfo.FIELDsaveFileVersion;
		private const string STARTconfig = lutils.STFLD + TABLEchannelConfig + LOR4SequenceInfo.FIELDchannelConfigFileVersion;
		...
		private const string STARTloop = lutils.STFLD + LOR4Loop.FIELDloop + lutils.SPC;
		private const string STARTaniRow = lutils.STFLD + LORAnimationRow4.FIELDrow + lutils.SPC + LORAnimationRow4.FIELDindex;
		private const string STARTaniCol = lutils.STFLD + LOR4AnimationColumn.FIELDcolumnIndex;
				...
				if (!reader.EndOfStream)
				{
					...
					else
					{
						...
						if (!reader.EndOfStream)
						{
							...
							{
								info = new LOR4SequenceInfo(this, lineIn);
												...
												else // Not a regular channel
												{
													...
													else // Not a regular channel
													{
														...
														if (li > 0)
														{
															...
															LOR4Channel ch = (LOR4Channel)Members.bySavedIndex[csi];
															...
															ch = (LOR4Channel)Members.bySavedIndex[csi];
															...
															else // Not a LOR4ChannelGroup
															{
																...
																if (li > 0)
																{
																	...
																	if (li < 0)
																	{
																		...
																		{
																			int isl = lutils.getKeyValue(lineIn, lutils.FIELDsavedIndex);
																			iLOR4Member mm = Members.bySavedIndex[isl]
; lastCosmic.Members.Add(mm);
																		...
																		if (li > 0)
																		{
																			...
																			if (li < 0)
																			{
																				...
																				while (li > 0)
																				{
																					...
																					{
																						iLOR4Member SIMem = Members.bySavedIndex[isi];
																						if (SIMem != null)
																						{
		...
		private int WriteRGBchannel(LOR4RGBChannel theRGBchannel, bool selectedOnly, bool noEffects, LOR4MemberType itemTypes)
		{
			if (!theRGBchannel.Written)
			{
				if ((!selectedOnly) || (theRGBchannel.Selected))
				{
					...
					if ((itemTypes == LOR4MemberType.Items) || (itemTypes == LOR4MemberType.Items) || (itemTypes == LOR4MemberType.RGBChannel))
					{
		...
		private int WriteItemsList(LOR4Membership itemIDs, bool selectedOnly, LOR4MemberType itemTypes)
		{
			...
			if (itemIDs.Owner.MemberType == LOR4MemberType.Track)
			{
				lineOut += lutils.TABLEchannel;
				leader += lutils.TABLEchannel;
			}
			if (itemIDs.Owner.MemberType == LOR4MemberType.ChannelGroup)
			{
			...
			foreach (iLOR4Member subID in itemIDs.Items)
			{
				...
				if (subID.Written)  // The item itself should have already been written!
				{
					...
					if (!selectedOnly || sel)
					{
						if ((itemTypes == subID.MemberType) || (itemTypes == LOR4MemberType.Items))
						{
		...
		public int WriteItem(int SavedIndex, bool selectedOnly, bool noEffects, LOR4MemberType theType)
		{
			...
			iLOR4Member member = Members.bySavedIndex[SavedIndex];
			if (!member.Written)
			{
		...
		public int ReadClipboardFile(string existingFilename)
		{
			...
			while ((lineIn = reader.ReadLine()) != null)
			{
				...
				if (pos1 > 0)
				{
					...
					ef.Intensity = lutils.getKeyValue(lineIn, lutils.SPC + LOR4Effect.FIELDintensity);
					ef.startIntensity = lutils.getKeyValue(lineIn, LOR4Effect.FIELDstartIntensity);
					ef.endIntensity = lutils.getKeyValue(lineIn, LOR4Effect.FIELDendIntensity);
		...
		public LOR4Channel FindChannel(string channelName)
		{
			LOR4Channel ret = null;
			iLOR4Member member = Members.Find(channelName, LOR4MemberType.Channel, false);
			if (member != null)
			{
				ret = (LOR4Channel)member;
			}
		...
		public LOR4RGBChannel FindRGBchannel(string rgbChannelName)
		{
			LOR4RGBChannel ret = null;
			iLOR4Member member = Members.Find(rgbChannelName, LOR4MemberType.RGBChannel, false);
			if (member != null)
			{
				ret = (LOR4RGBChannel)member;
			}
		...
		public LOR4ChannelGroup FindChannelGroup(string channelGroupName)
		{
			LOR4ChannelGroup ret = null;
			iLOR4Member member = Members.Find(channelGroupName, LOR4MemberType.ChannelGroup, false);
			if (member != null)
			{
				ret = (LOR4ChannelGroup)member;
			}
		...
		{
			LORTrack4 ret = null;
			iLOR4Member member = Members.Find(trackName, LOR4MemberType.Track, false);
			if (member != null)
			{
		...
		public LOR4Channel ParseChannel(string lineIn)
		{
			LOR4Channel chan = new LOR4Channel("");
			chan.SetParent(this);
		...
		public LOR4Channel CreateChannel(string theName)
		{
		...
		public LOR4RGBChannel ParseRGBchannel(string lineIn)
		{
			LOR4RGBChannel rch = new LOR4RGBChannel("");
			rch.SetParent(this);
		...
		public LOR4RGBChannel CreateRGBchannel(string theName)
		{
			// Does NOT check to see if a channel with this name already exists
			// Therefore, allows for duplicate channel names (But they will have different SavedIndexes)
			LOR4RGBChannel rch;
			rch = new LOR4RGBChannel(theName, lutils.UNDEFINED);
		...
		public LOR4ChannelGroup ParseChannelGroup(string lineIn)
		{
			LOR4ChannelGroup chg = new LOR4ChannelGroup("");
			chg.SetParent(this);
		...
		public LOR4Cosmic ParseCosmicDevice(string lineIn)
		{
			LOR4Cosmic cos = new LOR4Cosmic("");
			cos.SetParent(this);
		...
		public LOR4ChannelGroup CreateChannelGroup(string theName)
		{
			...
			LOR4ChannelGroup chg;
			chg = new LOR4ChannelGroup(theName, lutils.UNDEFINED);
		...
		public LOR4Cosmic CreateCosmicDevice(string theName)
		{
			...
			LOR4Cosmic dev;
			dev = new LOR4Cosmic(theName, lutils.UNDEFINED);
		...
		public LORTrack4 ParseTrack(string lineIn)
		{
			...
			if (myAltSavedIndex > lutils.UNDEFINED)
			{
				...
				//iLOR4Member tg = Members.bySaveID[tr.AltSavedIndex];
				LORTimings4 tg = null;
				for (int i = 0; i < TimingGrids.Count; i++)
				{
		...
		{
#if DEBUG
			string msg = "LOR4Sequence.CreateTimingGrid(" + theName + ")";
			Debug.WriteLine(msg);
#endif
		...
		public new iLOR4Member Clone()
		{
			...
			{
				LOR4MemberType mt = Members.bySavedIndex[idx].MemberType;
				if (mt == LOR4MemberType.Channel)
				{
					LOR4Channel newCh = (LOR4Channel)Members.bySavedIndex[idx].Clone();
