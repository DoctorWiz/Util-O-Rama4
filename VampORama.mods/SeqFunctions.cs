...
namespace UtilORama4
{
	public static class SequenceFunctions
	{
		...
		*/
		...
		public static int NoteColor_DEPRECIATED(int noteNum)
		// Use ChannelColor() instead	
		{
		...
		public static LOR4ChannelGroup CreatePolyChannels(iLOR4Member parent, string prefix, bool useGroups)
		{
			// Returns an array of Channels of (empty) channels for Polyphonic Transcription

			//string chPrefix = "Note ";
			string dmsg = "";
			//LOR4Channel chan;
			int octave = 0;
			int lastOctave = 0;
			//LOR4Membership parentSubs = new LOR4Membership(Sequence);
			LOR4ChannelGroup grp = new LOR4ChannelGroup(parent,"null");
			LOR4Channel[] polyChannels = null;
			Array.Resize(ref polyChannels, MusicalNotation.noteNamesUnicode.Length);
			//if (useGroups)
			//{
			//	LOR4ChannelGroup grp = parent.FindChannelGroup(prefix + MusicalNotation.octaveNamesA[octave], parent);
			...
			for (int n = 0; n < MusicalNotation.noteNamesUnicode.Length; n++)
			{
				...
				LOR4Channel chan = new LOR4Channel(parent,prefix + MusicalNotation.noteNamesUnicode[n]);
				chan.color = ChannelColor(n);
		...
		public static LOR4ChannelGroup[] CreatePolyOctaveGroups()
		{
			...
			for (int n = 0; n < MusicalNotation.noteNamesUnicode.Length; n++)
			{
				LOR4ChannelGroup grp = new LOR4ChannelGroup(polyGroups[n],prefix + MusicalNotation.octaveNamesA[n]);
			}
			return polyGroups;
		}
		...
		public static LOR4ChannelGroup PutNonEmptyPolyChannelsIntoPolyOctaveGroups(LOR4Channel[] polyChannels, LOR4ChannelGroup[] polyGroups)
		{
			...
			int err = 0;
			System.Diagnostics.Debugger.Break();
			LOR4Track trk = null; // seq.findTrack(vamps.MASTERTRACK);
			LOR4ChannelGroup parentGroup = new LOR4ChannelGroup(trk, "Polyphonic Transcription");
			int keepCount = 0;
