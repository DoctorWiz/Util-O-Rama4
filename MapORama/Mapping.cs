	...
	*/
		...
		public int LoadSourceFile(string sourceChannelFile)
		{
			...
			if (err < 100)
			{
				...
				this.Text = Path.GetFileName(sourceFile) + " - " + title;
		...
		private int SaveMap(string fileName)
		{
			...
			for (int i = 0; i < seqDest.Channels.Count; i++)
			{
				...
				{
					lineOut.Append(LOR4SeqEnums.MemberTypeName(destMember.MemberType));
					lineOut.Append(",");
		...
		private string LoadApplyMapFile(string fileName)
		{
			...
			string mfile = "";
			string sfile = "";
			//long finalAlgorithms = Settings.Default.FuzzyFinalAlgorithms;
			//double minPreMatch = Settings.Default.FuzzyMinPrematch;
			//double minFinalMatch = Settings.Default.FuzzyMinFinal;
			//long preAlgorithm = Settings.Default.FuzzyPrematchAlgorithm;
			...
			if (false) // See no reason to do Tracks & Groups
			{
				// Channel Groups
				for (int i = 0; i < seqDest.ChannelGroups.Count; i++)
				{
			...
			if (false) // See no reason to do Groups and Tracks
			{
				// Channel Groups
				for (int i = 0; i < seqSource.ChannelGroups.Count; i++)
				{
								...
								if (li > 0)
								{
									...
									if (!reader.EndOfStream)
									{
										...
										if (li > 0)
										{
											...
											if (seqDest.AllMembers.BySavedIndex[destSI].SelectedState != CheckState.Checked)
											{
												if (seqDest.AllMembers.BySavedIndex[destSI].MemberType == destType)
												{
													...
													foundID = FuzzyFindName(destName, seqDest, destType);
													if (foundID != null)
													{
													...
													if (destSI > LOR4Admin.UNDEFINED)
													{
														if (sourceSI < seqSource.AllMembers.BySavedIndex.Count)
														{
															...
															{
																if (seqSource.AllMembers.BySavedIndex[sourceSI].SelectedState != CheckState.Checked)
																{
																	...
																	foundID = FuzzyFindName(sourceName, seqSource, destType);
																	if (foundID != null)
																	{
		...
		private void SaveNewMappedSequence()
		{
			...
			dlgFileSave.Filter = "Musical Sequences (*.lms)|*" + LOR4Admin.EXT_LMS;
			dlgFileSave.Filter += "|Animated (no music) Sequences (*.las)|*" + LOR4Admin.EXT_LAS;
			dlgFileSave.Filter += "|eXtensible Markup Language (*.xml)|*.xml";
				...
				if (seqSource.Channels.Count > 0)
				{
					if (File.Exists(mapFile))
					{
						...
						if (mapFileLineCount > 2)
						{
							...
							{
								//? Do we need to update this or not?
								//cboMappingFile.Text = Path.GetFileName(mapFile);
								LoadApplyMapFile(mapFile);
		...
		private void AutoMap()
		{
			...
			pp = 0;
			string logFile = Fyle.GetUserTempPath + "Fuzzy.log";
			...
			// Second Pass, try to match by Exact Name
			int mcn = AutoMapMembersByExactName(seqDest.AllMembers);
			mappedMemberCount += mcn;
			// Third Pass, try to match by Fuzzy Name
			int mcc = AutoMapMembersByFuzzyName(seqDest.AllMembers);
		...
		private int AutoMapMembersByExactName(LOR4Membership destMembers)
		{
			int matchCount = 0;
			for (int memidx = 0; memidx < destMembers.Count; memidx++)
			{
		...
		private int AutoMapMembersByFuzzyName(LOR4Membership destMembers)
		{
			int matchCount = 0;
			for (int memidx = 0; memidx < destMembers.Count; memidx++)
			{
		...
		public iLOR4Member FuzzyFindName(string theName, LOR4Sequence sequence, LOR4MemberType theType)
		{
			...
			bool useFuzzy = userSettings.FuzzyUse;
			bool writeLog = userSettings.FuzzyWriteLog;
			...
			if (writeLog)
			{
				logFile = Fyle.GetUserTempPath + "Fuzzy.log";
				writer = new StreamWriter(logFile, true);
				writer.WriteLine("");
				lineOut = "Looking for     \"" + theName + "\" in ";
				lineOut += LOR4SeqEnums.MemberTypeName(theType) + "s in ";
			...
			if (theType == LOR4MemberType.Channel)
			{
				...
				} // End for loop thru all channels in sequence
			} // End if theType is a Channel
			...
			if (count > 0)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					thisScore = source.FuzzyScoreAccurate(target);
					//watch.Stop();
				...
				if (scores[count - 1] > FuzzyFunctions.SUGGESTED_MIN_FINAL_SCORE)
				{
				...
				else
				{
					if (writeLog)
					{
						lineOut = "No Final Matches meet the minimum score of " + FuzzyFunctions.SUGGESTED_MIN_FINAL_SCORE.ToString();
						writer.WriteLine(lineOut);
					}
		...
		public iLOR4Member FuzzyFind(string theName, LOR4Membership children, bool ignoreSelected)
		{
			...
			for (int idx = 0; idx < children.Items.Count; idx++)
			{
				...
				if ((child.SelectedState != CheckState.Checked) || (!ignoreSelected))
				{
					bestScore = 0D;
					bestIndex = -1;
					// get a quick prematch score
		...
		private bool MapMembers(iLOR4Member sourceMember, iLOR4Member destMember, bool andChildren = true)
		{
			...
			if (sourceMember.MemberType == destMember.MemberType)
			{
				...
				destMember.SelectedState = CheckState.Checked;
				sourceMember.SelectedState = CheckState.Checked;
			...
			{
				string msg = "Trying to map " + LOR4SeqEnums.MemberTypeName(sourceMember.MemberType) + "	'" + sourceMember.Name + "'";
				msg += " to " + LOR4SeqEnums.MemberTypeName(destMember.MemberType) + " '" + destMember.Name + "'!";
				Fyle.BUG(msg);
			}
		...
		{
			bool success = false;
			if ((sourceMember.SelectedState == CheckState.Checked) && (destMember.SelectedState == CheckState.Checked))
			{
				if (sourceMember.MemberType == destMember.MemberType)
				{
					if (destMember.MapTo != null)
					{
						if (destMember.MapTo.ID == sourceMember.ID)
						{
							...
							for (int dm = 0; dm < destMapList.Count; dm++)
							{
								...
								{
									destMapList[dm].SelectedState = CheckState.Unchecked;
									destMapList.RemoveAt(dm);
									dm = destMapList.Count; // force exit of loop
								}
							}
							sourceMember.ZCount = destMapList.Count;
							sourceMember.SelectedState = CheckState.Unchecked;
							destMember.SelectedState = CheckState.Unchecked;
							TreeUtils.EmboldenMember(sourceMember, true);
		...
		{
			member.SelectedState = CheckState.Unchecked;
			member.MapTo = null;
