...
namespace UtilORama4
{
	public partial class frmVamp : Form
	{
		...
		//private LORTrack4 tuneTrack = null;
		...
		{
			string songFileTemp = lutils.GetAppTempFolder() + "song.mp3";
			int errs = lutils.SafeCopy(songFile, songFileTemp, true);
			if (errs == 0)
			{
		...
		*/
		...
		private string VampIt(string VampParams, string configFile)
		{
			...
			const string ERRtuneTrack = "), in LORTrack4 #";
				...
				//{
					...
					if (dr != DialogResult.Cancel)
					{
						...
						if ((!chkReuse.Checked) || (!Fyle.Exists(resultsFile)))
						{
							...
							//Level ll = Level.LORSeqInfo4; // Initialize to default
	...
	/*
				...
				if (ppos > xUtils.UNDEFINED)
				{
					...
					if (millisecs > lastOnset)
					{
						...
						if (parts.Length == 3)
						{
							...
							{
								onsetLabel = noteNamesUnicode[note];
							}
				...
				if (ppos > xUtils.UNDEFINED)
				{
					...
					if ((align < 1) || (millisecs > lastNote))
					{
						...
						if (parts.Length == 3)
						{
							...
							{
								noteLabel = noteNamesUnicode[note];
							}
							if (cboLabelsOnsets.SelectedIndex == 2)
							{
		...
		private void AddPolyTimings_Other(string prefix)
		{
			...
			Array.Resize(ref noteTimings, noteNamesUnicode.Length);
			for (int n = 0; n < noteNamesUnicode.Length; n++)
			{
				xTimings times = new xTimings(prefix + noteNamesUnicode[n]);
				noteTimings[n] = times;
